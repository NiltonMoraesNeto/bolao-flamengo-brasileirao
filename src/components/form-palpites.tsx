...
const FormularioPalpite: React.FC<FormularioPalpiteProps> = ({
  ...
  const calcularPontuacao = (resultado: Resultado[], palpite: Palpite) => {
    let pontos = 0;
    if (
      palpite.golsCasa === resultado[0]?.golsCasa &&
      palpite.golsFora === resultado[0]?.golsFora
    ) {
      pontos = 25;
    } else if (
      (palpite.golsCasa > palpite.golsFora &&
        resultado[0]?.golsCasa > resultado[0]?.golsFora) ||
      (palpite.golsCasa < palpite.golsFora &&
        resultado[0]?.golsCasa < resultado[0]?.golsFora) ||
      (palpite.golsCasa === palpite.golsFora &&
        resultado[0]?.golsCasa === resultado[0]?.golsFora)
  ...
  useEffect(() => {
    const loadPalpites = async () => {
      ...
      if (responsePalpites) {
        const palpitesComPontos = await Promise.all(
          responsePalpites.map(async (palpite: PalpitesTabela) => {
    ...
    const verificarJogoAgora = async () => {
      ...
      const jogoDate = new Date(responseJogo[0].data.replace(' ', 'T')); // Transformando a string em Date válida
    ...
    // Configura um intervalo para verificar a cada 60 segundos
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          className={`cursor-pointer inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-red-500 hover:bg-gray-600 ${isJogoAgora ? 'bg-gray-600' : 'bg-black'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500`}
            ...
            {selectedPalpite?.map((jogo) => (
              <tr key={jogo.id} className="hover:bg-gray-100">
                ...
                <td className="py-2 px-4 border-b border-gray-200 text-left text-sm text-gray-900">
                  <button
                    type="button"
                    className={`cursor-pointer ${jogoRealizado ? 'text-red-200' : 'text-red-600'} hover:text-red-800 focus:outline-none focus:underline`}
